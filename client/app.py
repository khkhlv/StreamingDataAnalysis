...
templates = Jinja2Templates(directory="client")  # ← КРИТИЧЕСКИ ВАЖНО: "client" а не "templates"
...
def kafka_reader():
    ...
    try:
        consumer = KafkaConsumer(
            ...
            auto_offset_reset='latest',  # ← КАК В ВАШЕМ РАБОЧЕМ КОДЕ
        ...
        for msg in consumer:
            try:
                # === ВАШ РАБОЧИЙ ПАРСИНГ ВЛОЖЕННОЙ СТРУКТУРЫ ===
                data = msg.value
